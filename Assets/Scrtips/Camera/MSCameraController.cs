...
public class MSCameraController : Singletone<MSCameraController> {
	...
	void EnableCameras (int index){
		if (cameras.Length > 0) {
			...
			for (int x = 0; x < cameras.Length; x++) {
				if (cameras [x]._camera) {
					var camera = cameras[x]._camera;
					if (x == index)
					{
						camera.gameObject.SetActive(true);
						onCameraSwitch.Invoke(camera);
						//
						if (cameras [x].rotationType == MSACC_CameraType.TipoRotac.FirstPerson) {
							rotacX = 0.0f;
							rotacY = 0.0f;
						...
						if (cameras [x].rotationType == MSACC_CameraType.TipoRotac.FlyCamera_OnlyWindows) {
							camera.transform.position = cameras [lastIndex]._camera.transform.position;
